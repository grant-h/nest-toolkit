...
void line(void * fbmem, unsigned y, unsigned color)
{
  unsigned x;
  unsigned w = var.xres;
...
void wipe_screen(void * fbmem, unsigned color)
{
  unsigned h = var.yres;
  unsigned y;
  int bot;

  unsigned limit = (h-2)/2;
  
  fprintf(stderr, "wipe screen\n");

  for (y = 0, bot = h-1; y < limit; y++, bot--) {
    if(y >= 2)
      line(fbmem, y-2, 0);
    ...
    line(fbmem, y, color);
    if(y+1 < h)
    line(fbmem, y+1, color);


    if(bot-1 >= 0)
    line(fbmem, bot-1, color);
    if(bot >= 0)
    line(fbmem, bot, color);
...
static float fpsAddAve(int ticks);
void exitFunc(int sig)
{
  clear_screen(gFbmem, 0);

  fprintf(stderr, "Ave ticks %f\n", fpsAddAve(-1));
  exit(0);
}
...
int main(int argc, char** argv)
{
  ...
  for(i = 0; i < amount; i++)
  {
    ...
    //fprintf(stderr, "%f ", tstates[i].radius);
    tstates[i].initrad = tstates[i].radius;
    tstates[i].radiusStep = randFloat(4.5, 10.9);
  ...
  while(1) {
    ...
    {
      //clear_screen(ptr, 0x0);
      wipe_screen(ptr, 0xff0000);
  //sleep(1);
      for(i = 0; i < amount; i++) {
        color = randColor();
        //fprintf(stderr, "new tumble %x: ", color);

        memset(&tstates[i], 0, sizeof(tstates[i]));
        ...
        /*tstates[i].radius = 160.0-1-10*i;
        fprintf(stderr, "Should be %f ", tstates[i].radius);
        tstates[i].initrad = tstates[i].radius;
        fprintf(stderr, "is - %f ", tstates[i].radius);
